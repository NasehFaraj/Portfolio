...
export default function StackAndCapabilities({
  isArabic,
  variant = "light"
}: StackAndCapabilitiesProps) {
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-16">
        <Reveal>
          <div className="mb-10">
            <h2 className={`text-2xl font-semibold sm:text-3xl ${isDark ? "text-white" : "text-slate-900"}`}>
        ...
        <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,260px),1fr] sm:gap-8">
          <aside className="self-start w-full lg:sticky lg:top-24 lg:max-w-[260px]">
            <div className="flex flex-col gap-4">
              <Reveal>
                <div
                  className={`rounded-2xl p-4 shadow-sm transition-transform duration-200 ease-out hover:-translate-y-0.5 hover:shadow-lg sm:p-5 ${
                    isDark
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {tech.core.map((item) => {
                      ...
                      return (
                        ...
                        >
                          {Icon ? (
                            <Icon className={`h-4 w-4 ${isDark ? "text-nest-400" : "text-nest-600"}`} />
                          ) : null}
                          <span
              ...
              return (
                <Reveal key={capability.id} delay={index * 0.06}>
                  <div className="space-y-6">
                    <div
                      className={`rounded-2xl p-4 shadow-sm transition-transform duration-200 ease-out hover:-translate-y-0.5 hover:shadow-lg sm:p-6 ${
                        isDark
                          ? "border border-white/10 bg-white/5"
                          : "border border-rose-200 bg-white/80"
                      }`}
                    >
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                          ...
                          <ul
                            className={`mt-3 space-y-2 leading-relaxed list-disc ${
                    ...
                    {index < content.capabilities.length - 1 ? (
                      <div className={`h-px ${isDark ? "bg-white/10" : "bg-slate-200/70"}`} />
                    ) : null}
                  </div>
                </Reveal>
              );
